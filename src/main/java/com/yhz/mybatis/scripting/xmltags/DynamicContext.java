...
public class DynamicContext {
    ...
    //PropertyAccessor是OGNL中的一个接口，用于定义Java对象属性访问的规范。PropertyAccessor接口
    // 定义了访问Java对象属性的方法，包括获取属性值、设置属性值和检查属性是否存在。
    //在使用OGNL访问Java对象的属性时，需要指定一个PropertyAccessor来解析属性访问表达式，
    // 并根据实际的对象类型和属性名称来获取属性值、设置属性值或检查属性是否存在。
    // OGNL默认提供了几个常见的PropertyAccessor实现，包括BeanPropertyAccessor、
    // IndexedPropertyAccessor、MapPropertyAccessor等，用于访问JavaBean、数组、集合、Map等
    // 类型的对象属性
