package com.yhz.mybatis.datasource.unpooled;
...
/**
 * 无池化的数据源链接实现比较简单，核心在于 initializerDriver 初始化驱动中使用了
 * Class.forName 和 newInstance 的方式创建了数据源链接操作。
    ...
    private synchronized void initializerDriver() throws SQLException {
        if (!registeredDrivers.containsKey(driver)) {
            try {
                ...
                //在Java中，通过DriverManager来获取Connection对象，DriverManager会负
                // 责根据用户提供的JDBC URL来确定需要使用哪个驱动程序，并返回相应的Connection对象。
                // 如果没有注册驱动程序，DriverManager是无法找到相应的驱动程序并创建Connection对象的。
