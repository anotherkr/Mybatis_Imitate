...
import com.yhz.mybatis.datasource.unpooled.UnpooledDataSource;
...
/**
 * PooledDataSource是MyBatis中连接池的实现，它通过维护两个连接列表，分别表示空闲连接和
 * 活跃连接来管理数据库连接的使用。当MyBatis需要连接时，先从空闲连接列表中获取连接，如果空闲连接列表为空，
 * 再从数据库中获取连接，同时维护活跃连接列表的长度。当连接使用完毕时，将连接返回到连接池中，放入空闲连接列表中，
 * 等待下一次使用。为了保证连接的可用性和连接的有效性，PooledDataSource采取了一系列的措施，例如定时检查连接是否
 * 可用、定时清理空闲连接等，从而保证了连接池的高效使用和连接的有效性。
 * @author yanhuanzhan
...
public class PooledDataSource implements DataSource {
    ...
    /**
     * 数据源回收的处理中，核心在于判断链接是否有效，以及进行相关的空闲链接校验，
        ...
        synchronized (state) {
            ...
            if (connection.isValid()) {
                ...
                if (state.idleConnections.size() < poolMaximumIdleConnections && connection.getConnectionTypeCode() == expectedConnectionTypeCode) {
                   ...
                   //当一个连接被从连接池中取出并使用时，如果该连接未开启自动提交模式，则可能处于一个未提交的事务中。
                    // 在这种情况下，当这个连接被归还到连接池中时，需要进行回滚操作，以确保数据的一致性。因为下一个
                    // 使用该连接的线程可能会期望得到一个新的、干净的连接。如果该连接处于一个未提交的事务中，
                    // 可能会对下一个使用该连接的线程造成干扰，导致数据不一致。因此，为了避免这种情况，
                    // 需要对未提交的事务进行回滚操作
    ...
    private PooledConnection popConnection(String username, String password) throws SQLException {
        ...
        while (conn == null) {
            //在加锁的代码块中通过判断是否还有空闲链接进行返回，如果没有则会判断活跃连接数是否充足，
            // 不充足则进行创建后返回。在这里也会遇到活跃链接已经进行循环等待的过程，最后再不能获取则抛出异常。
            synchronized (state) {
                // 如果有空闲链接：返回第一个
